...
- (void)replyToHTTPRequest
{
	...
	if ([WebSocket isWebSocketRequest:request])
	{
		...
		else
		{
			...
			{
				ERROR(@"WebSocket forgot to set itself as socket delegate");
...
- (NSString *)filePathForURI:(NSString *)path allowDirectory:(BOOL)allowDirectory
{
	...
	if (documentRoot == nil)
	{
		ERROR(@"No configured document root");
		return nil;
	}
	...
	if (docRoot == nil)
	{
		ERROR(@"Document root is invalid file path");
		return nil;
	}
	...
	{
		ERROR(@"Request for file outside document root");
		return nil;
	}
...
- (void)handleVersionNotSupported:(NSString *)version
{
	...
	ERROR(@"HTTP Server: Error 505 - Version Not Supported: %@ (%@)", version, [self requestURI]);
...
- (void)handleInvalidRequest:(NSData *)data
{
	...
	ERROR(@"HTTP Server: Error 400 - Bad Request (%@)", [self requestURI]);
...
- (void)handleUnknownMethod:(NSString *)method
{
	...
	ERROR(@"HTTP Server: Error 405 - Method Not Allowed: %@ (%@)", method, [self requestURI]);
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
		...
		else
		{
			...
			if (expectsUpload)
			{
				...
				else
				{
					...
					{
						ERROR(@"Method expects request body, but had no specified Content-Length");
					...
					{
						ERROR(@"Unable to parse Content-Length header into a valid number");
			...
			else
			{
				if (contentLength != nil)
				{
					...
					{
						ERROR(@"Method not expecting request body had non-zero Content-Length");
	...
	else
	{
		...
		if (tag == HTTP_REQUEST_CHUNK_SIZE)
		{
			...
			if (errno != 0)
			{
				ERROR(@"Method expects chunk size, but received something else");
		...
		else if (tag == HTTP_REQUEST_CHUNK_TRAILER)
		{
			...
			{
				ERROR(@"Method expects chunk trailer, but is missing");
...
- (void)responseHasAvailableData:(NSObject<HTTPResponse> *)sender
{
	...
	dispatch_async(connectionQueue, ^{ @autoreleasepool {
		...
		{
			ERROR(@"Sender is not current httpResponse");
			return;
		}
