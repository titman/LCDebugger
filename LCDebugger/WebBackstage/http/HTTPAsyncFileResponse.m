...
- (id)initWithFilePath:(NSString *)fpath forConnection:(HTTPConnection *)parent
{
	if ((self = [super init]))
	{
		...
		if (filePath == nil)
		{
			ERROR(@"Init failed - Nil filePath");
		...
		{
			ERROR(@"Init failed - Unable to get file attributes. filePath: %@", filePath);
...
- (BOOL)openFileAndSetupReadSource
{
	...
	if (fileFD == NULL_FD)
	{
		ERROR(@"Unable to open file. filePath: %@", filePath);
	...
	dispatch_source_set_event_handler(readSource, ^{
		...
		{
			//ERROR(@"Read EOF on file(%@)", filePath);
...
- (void)setOffset:(UInt64)offset
{
	...
	if (result == -1)
	{
		ERROR(@"lseek failed - errno(%i) filePath(%@)", errno, filePath);
